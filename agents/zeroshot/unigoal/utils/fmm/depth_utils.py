...
def show_depth(depth, fig_id=1):
    """
    depth: 可以是 (H,W)、(H,W,1)、(1,H,W) 等，numpy 或 torch 都行
    """
    # 转 numpy
...
# 用法：
# show_depth(depth)  # depth 是 (1,160,120) 或其它形状都可
...
def get_point_cloud_from_z_t(Y_t, camera_matrix, device, scale=1):
    ...
    Outputs:
        ...
        XYZ is ...xHxWx3
    """
    #print(Y_t,Y_t.shape,camera_matrix)
    #exit()
    #show_depth(Y_t)
...
def transform_camera_view_t_Y(
        XYZ, dist, camera_elevation_degree, device):
...
def transform_pose_t(XYZ, current_pose, device):
    ...
    """
    R = get_r_matrix([0., 0., 1.], angle=current_pose[2] - np.pi / 2.)
    #print("angle",current_pose[2] - np.pi / 2.,"current_pose",current_pose)
    #angle 0.0 current_pose [250, 0, 1.5707963267948966]
