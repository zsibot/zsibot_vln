...
def extract_assistant_text(raw: str) -> str:
    """
    Post-process the raw model output to return only the assistant's answer.

    If the model outputs something like:
        'Robot:\\nUser:\\n...\\nAssistant: Dark brown cabinet ...'
    we strip everything before the LAST 'Assistant:' and return the rest.
    """
    marker = "Assistant:"
    idx = raw.rfind(marker)
    if idx != -1:
        # Take text after 'Assistant:'
        return raw[idx + len(marker):].strip()
    # Fallback: no marker, just trim whitespace
    return raw.strip()
...
def chat_completions(req: ChatCompletionRequest):
    ...
    raw_answer: str = generated_texts[0]

    # Only keep the pure assistant answer text
    answer: str = extract_assistant_text(raw_answer)
